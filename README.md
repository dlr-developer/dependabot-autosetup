...
Make sure you have Git Bash installed (comes with Git for Windows). Open PowerShell or Command Prompt and run:
...
Or in a standard Command Prompt:
```cmd
...
This installs all five files -- `dependabot-autosetup.sh`, `dependabot-autosetup.bat`, `install-dependabot-autosetup-manager.sh`, `README.md`, and `unblock-screenshot-windows.png` -- into `./scripts/dependabot-autosetup`, nested in its own folder so it never collides with other scripts already in your project's `scripts/` folder. All of them require [Git for Windows](https://git-scm.com/download/win) to be installed (Mac/Linux already have bash built in).
