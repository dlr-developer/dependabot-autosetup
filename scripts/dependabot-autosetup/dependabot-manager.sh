...
while true; do
  ...
  echo -e "     Show all open Dependabot PRs across all your repositories in one list,"
  echo -e "     estimate update risks, and choose which ones to merge from here."
  echo ""
  echo -e "  ${C_HEADER}2)${C_RESET} Run Multi-Repo Bulk Setup / Installer"
  echo -e "     Scan configured directories for Git repositories. See which ones have"
  echo -e "     dependabot-autosetup installed, and bulk install or update them."
  echo ""
  echo -e "  ${C_HEADER}3)${C_RESET} Configure Scan Directories"
  echo -e "     Add, delete, or review the parent directories on your machine where"
  echo -e "     your projects are located."
  echo ""
